...
event SponsorReEntry(address indexed sponsor, address indexed from, uint8 packageId);
    ...
    function _level5ReEntry(uint8 packageId, address userA) internal {
        ...
        _placeInMatrixForPackage(TRACK_MATRIX, packageId, phantomNode, placementAnchor);
    ...
    function _releaseDirectIncome(address buyer, uint8 packageId, uint256 fullAmount) internal {
        ...
        address currentSponsor = _findEligibleSponsorUplineCapped(rawSponsor, packageId, 100);
        ...
        address sourceUser = buyer;

        while (currentSponsor != address(0)) {
            sponsorCycleCountByPkg[packageId][currentSponsor]++;
            uint256 count = sponsorCycleCountByPkg[packageId][currentSponsor];
            
            bool upgraded = (packageId >= MAX_PACKAGE) || (sponsorPackageId[currentSponsor] > packageId);
            (uint256 sponsorNet, uint256 adminCut) = _splitSponsorShare(sponsorShare);

            if (count == 1 || count == 2) {
                usdt.safeTransfer(adminWallet, adminCut);
                uint256 paidNet = _payCapped(currentSponsor, sponsorNet);
                userTotalDirectIncome[currentSponsor] += paidNet;
                ...
                sponsorCycleCountByPkg[packageId][currentSponsor] = 0;
                recycleCount[currentSponsor]++;
                
                address eligibleUpline = _findEligibleSponsorUpline(currentSponsor, packageId);
                
                phantomCounter++;
                address phantom = address(uint160(uint256(keccak256(
                    abi.encodePacked("SPONSOR_RE", packageId, currentSponsor, phantomCounter, block.timestamp)
                ))));
                
                realOwnerByPkg[packageId][phantom] = currentSponsor;
    ...
    function _findEligibleSponsorUplineCapped(address startFrom, uint8 packageId, uint256 maxHops) internal view returns (address) {

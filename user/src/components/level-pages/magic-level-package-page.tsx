...
// import { fetchUserStringId } from "@/lib/auth-api";

// const SPONSOR_PACKAGE_PRICES = magicLevelPackages;
// const MAX_LEVEL = 100; // team-level pagination ki upper limit
...
//       }
//       try{
//         const user_id = await fetchUserStringId(address);
//         setUserId(user_id);
//       }
//       catch(err){
//         console.log(err);
...
export const MagicLevelPackagePage = ({
  ...
  //     try {
  //       const [user_id, depthResult] = await Promise.all([
  //         fetchUserStringId(address),
  //         fetchTeamLevelByDepth(address, currentTeamLevel),
  //       ]);
  ...
  //           (sum, row) => sum + row.totalBussiness,
  //           0,
  //         );

  //         setTeamMembers(rows);
  //         setTotalRevenue(`${revenue.toFixed(3)} USDT`);
  //       }
  //     } catch (err) {
  //       console.log(err);
  //     } finally {
  ...
  //       return;
  //     }
  //     setIsDataLoading(true); // Yahan bhi rakha — direct URL change/back-forward navigation ke liye bhi kaam karega
  //     try {
  //       const [user_id, depthRows] = await Promise.all([
  //         fetchUserStringId(address),
  //         fetchTeamLevelByDepth(address, currentTeamLevel),
  //       ]);
  ...
  //         const matched = depthRows.filter(
  //           (row) => row.packageId === packageNum,
  //         );

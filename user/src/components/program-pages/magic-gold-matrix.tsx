...
const MagicGoldMatrix = ({ program, walletAddress }: pageProps) => {
  ...
  useEffect(() => {
    async function loadAllData() {
      ...
      try {
        ...
        const { numericId,stringId } = await fetchPackageAndId(activeWalletAddress);
        ...
        const formattedHistory: TransactionRow[] = historyData
          .filter((item: any) => item.track === "MATRIX" && Number(item.packageId) === currentPkg)
  ...
  const levelsData: LevelDataXGold[] = packagePrices.map((price, index) => {
    ...
    return {
      ...
      isDamage: !isUnlocked,
      tree: isUnlocked ? (metrics.tree && metrics.tree.length > 0 ? metrics.tree : [
        generateSlots(0, 2),
        generateSlots(2, 4),
        generateSlots(6, 8),
        generateSlots(14, 18)
